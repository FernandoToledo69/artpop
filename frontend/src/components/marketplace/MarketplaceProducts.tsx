...
export default function MarketplaceProducts() {
  ...
  }, [category, municipality, products, query, technique]);
  ...
  return (
    <section className="marketplace-section" aria-labelledby="marketplace-title">
      ...
      {filteredProducts.length === 0 ? <div className="catalog-empty"><h3>Nenhuma obra encontrada</h3><p>Ajuste os filtros para descobrir outras criações.</p></div> : <div className="marketplace-grid">{filteredProducts.map((product) => <article className="marketplace-card" key={product.id}><div className={`marketplace-image marketplace-image-${product.category}`}><span aria-hidden="true">{product.mainImage ? <img src={product.mainImage} alt={`Imagem da obra ${product.title}`} /> : product.category === 'ceramica' ? 'FORMA' : product.category === 'madeira' ? 'RAIZ' : product.category === 'textil' ? 'FIO' : product.category === 'joalheria' ? 'LUZ' : 'TRAÇO'}</span></div><div className="marketplace-card-body"><p className="marketplace-category">{categoryLabels[product.category]} · {product.technique}</p><h3>{product.title}</h3><p className="marketplace-city">{product.artisanCity || 'Município não informado'} · {product.stock} disponíveis</p><strong>R$ {product.price.toFixed(2).replace('.', ',')}</strong></div></article>)}</div>}
    </section>
  );
}